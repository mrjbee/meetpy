...
from features.command import ListCommands, GetCommandDetails, ExecuteCommand
...
class Places(ActionHandler):
    def do_get(self, id_map, header_map):
        ...
        places = executor.execute(fs.GetAvailablePlaces)
        respond_json = []
        for place in places:
...
class Files(ActionHandler):
    def do_get(self, id_map, header_map):
        ...
        if parent_path is None:
            return INVALID
        assert isinstance(executor, RequestExecutor)
        sub_files = executor.execute(fs.ExploreFiles, parent_path)
