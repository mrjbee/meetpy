...
public class GetTaskDetails extends UserCaseSupport<TaskIdentifier, Task> {
    ...
    protected Task executeImpl(TaskIdentifier request) {
        ...
        try {
            ...
            Object progressObj = response.body.asObject().value("progress", Object.class);
            float progress = 1f;
            if (progressObj instanceof Double){
                progress = ((Double) progressObj).floatValue();
            }
            ...
            return new Task(request,title, description, status, (float) progress);
